...
import { categoryIdSchema, createCategorySchema } from '../validations/category.validation';
...
categoryRoutes.patch('/:categoryId', async (req, res) => {
  const { data: categoryIdData, error: categoryIdError } = categoryIdSchema.safeParse(req.params)
  if (categoryIdError) {
...
categoryRoutes.get('/', async (req, res) => {
  ...
  success(res, "Category created", 200, { categories })
})
