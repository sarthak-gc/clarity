...
authRoutes.post("/login", async (req, res) => {
  ...
  success(res, "Logged in successfully", 200, { token, userId: userExists.id });
});
authRoutes.post("/register", async (req, res) => {
  ...
  if (userExists) {
    throw new ConflictError(
      "User with that username already exists. Try different username",
    );
  }
  ...
  success(res, "user created", 201, { token, userId: user.id });
});
